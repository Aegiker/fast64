...
def ootAddLimbRecursively(
    ...
    # Only 4 and 11 are documented, but Epona also has 0 and 5.
    # I think 5 is for control bones, and 0 is for skin transform bones
    # 0 and 5 should make no difference in game, might be an export thing
...
def ootBuildSkeleton(
    ...
    isLOD = ootAddLimbRecursively(0, skeletonData, obj, armatureObj, transformMatrix, None, f3dContext, useFarLOD, useSkinLimbs)
     ...
     # Dictionary needs to be generated ahead of time
     # usually its entries are generated by parsed DLs and they get used for matrix stuff
     # which works fine except when there are bones that don't have a DL but do have a matrix
     # it may cause a KeyError if this happens
    if useSkinLimbs:
        for idx, limb in enumerate(f3dContext.limbList):
            f3dContext.limbToBoneName[f3dContext.getLimbName(idx)] = f3dContext.getBoneName(idx)
